<div class="p-4">
    ...
    <form action="{{ route('update.assignment', [$subject->id, $assignment->id]) }}" id="update_assignment">
        ...
        <br>
        <input required type="text" placeholder="Naam opdracht" name="assignment_name" value="{{ $assignment->assignment_name }}" class="border m-2 p-2 rounded dark:text-black">
        <br>
        <label for="assignment_info" class="p-2">Opdracht info</label>
        <br>
        <textarea required rows="3" cols="25" form="update_assignment" name="assignment_info" class="border mx-2 p-2 rounded dark:text-black">{{ $assignment->assignment_info }}
        </textarea>
        <br>
